...
NOCPS.prototype.getHost = function(mac, callback) {
  new Host(mac, this.client, function(err, host) {
    callback(err, host);
  });
};
...
NOCPS.prototype.findHost = function(hostname, callback) {
  ...
  this.getSubnets(undefined, undefined, function(error, subnets) {
    async.mapSeries(subnets.data, function(subnet, icallback) {
      self.getHosts(subnet.subnet, undefined, undefined, function(err, hosts) {
        for (var i = 0; i < hosts.data.length; i++) {
          if(hosts.data[i].hostname === hostname) {
            new Host(hosts.data[i].mac, self.client, function(host) {
              icallback(host);
            });
          }
        }
